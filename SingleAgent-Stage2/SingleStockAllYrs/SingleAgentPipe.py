...
T = 168#1008#10080#5040 #2520 #5000 #
TR = 42#2520 #???
...
def Visualize_dyn_comp2(timestamps, arr_lst_phidot, arr_lst_phi, ts, name, algo_lst, comment = None):
    ...
    if POWER == 2:
        ...
        power_type = "3on2"
    fname = f"single_{power_type}_{TR}_new"
    plt.savefig(f"{drive_dir}/Plots/{fname}.pdf", bbox_inches='tight')
    plt.close()
...
def transfer_learning(train_args, N_rounds = 5, n_train = 1, n_sample_lst = [128, 128], lr_lst = [1e-3, 1e-4], epoch_lst = [20000, 20000], retrain = False, report = True):
    if POWER == 2:
        cost = "quadratic"
    else:
        cost = "power"
    ...
    if POWER == 2:
        ...
        power_type = "power"
    log_dir = f"{drive_dir}/{power_type}_{TR}.txt"
                ...
                if train_args["pasting_algo"] == "deep_hedging":
                    if HIGH_DIM:
                        ...
                        if POWER == 2:
                            ...
                            model_name_dh = f"xdr_quad_{TR}_1"
                        else:
                            model_name = f"{pasting_algo}_power_42"
                            model_name_dh = f"xdr_power_{TR}_1"
                else:
                    if POWER == 2:
...
#            f.write(f"Seed = {i + 1}, Utility = {round(loss_eval_leading_order[0] * COEF_ / 1e9, 2)}\n")
    ...
    with open(log_dir, "a") as f:
        ...
        if not report:
            ...
            f.write(f"utility loss: {loss_eval_algo}\n")
            f.write(f"deep heding loss: {loss_eval_deep_hedging}\n")
            f.write(f"leading order loss: {loss_eval_leading_order}\n")
            if cost == "quadratic":
                f.write(f"ground truth loss: {loss_eval_ground_truth}\n")
...
train_args = {
    ...
    "retrain": False,
    "pasting_cutoff": T - 50,#2470,#4990,#454,#908,#9880,#4940, #404,#,#2510, #4800,
...
if train_args["algo"] == "pasting":
    ...
    model, loss_arr, prev_ts, curr_ts, train_args, model_dh = transfer_learning(train_args, N_rounds = 1, n_train = 1, n_sample_lst = [128, 500], lr_lst = [1e-3, 1e-4], epoch_lst = [10, 10])
else:
    model, loss_arr, prev_ts, curr_ts = training_pipeline(**train_args)
model.eval()
## TODO: Modify It!!!
...
## TODO: Modify It!!!
    phi_dot_stm_algo, phi_stm_algo, loss_eval_algo = evaluation(dW_STD_transfer, curr_ts, model, algo = train_args["algo"], cost = train_args["cost"], visualize_obs = 0, pasting_cutoff = train_args["pasting_cutoff"], pasting_T = train_args["pasting_T"], report = False)
    phi_dot_stm_leading_order, phi_stm_leading_order, loss_eval_leading_order = evaluation(dW_STD_transfer, curr_ts, None, algo = "leading_order", cost = train_args["cost"], visualize_obs = 0, pasting_cutoff = train_args["pasting_cutoff"], pasting_T = train_args["pasting_T"], report = False)
    phi_dot_stm_deep_hedging, phi_stm_deep_hedging, loss_eval_deep_hedging = evaluation(dW_STD_transfer, curr_ts, model_dh, algo = "deep_hedging", cost = train_args["cost"], visualize_obs = 0, pasting_cutoff = train_args["pasting_cutoff"], is_arr = False, pasting_T = train_args["pasting_T"], report = False)
    if cost == "quadratic":
        phi_dot_stm_ground_truth, phi_stm_ground_truth, loss_eval_ground_truth = evaluation(dW_STD_transfer, curr_ts, None, algo = "ground_truth", cost = train_args["cost"], visualize_obs = 0, pasting_cutoff = train_args["pasting_cutoff"], pasting_T = train_args["pasting_T"], report = False)
else:
...
vis_start = 0#400#900#9800#4900#400# #2400#4500
if POWER == 3/2 and TR == 2520:
    vis_start = 50
if train_args["algo"] == "pasting":
    day = int(TR - train_args["pasting_cutoff"] / T * TR)
    comment = f"Transfer at last {day} days"
else:
    comment = None
vis_id = 1#100
if cost == "quadratic":
#    Visualize_dyn_comp(TIMESTAMPS[(vis_start+1):], [phi_stm_algo[vis_id,(vis_start+1):,:], phi_stm_deep_hedging[vis_id,(vis_start+1):,:], phi_stm_leading_order[vis_id,(vis_start+1):,:], phi_stm_ground_truth[vis_id,(vis_start+1):,:]], curr_ts, "phi", [train_args["algo"], "deep_hedging", "leading_order", "ground_truth"], comment = comment)
#    Visualize_dyn_comp(TIMESTAMPS[(vis_start+1):], [phi_dot_stm_algo[vis_id,vis_start:,:], phi_dot_stm_deep_hedging[vis_id,vis_start:,:], phi_dot_stm_leading_order[vis_id,vis_start:,:], phi_dot_stm_ground_truth[vis_id,vis_start:,:]], curr_ts, "phi_dot", [train_args["algo"], "deep_hedging", "leading_order", "ground_truth"], comment = comment)
    Visualize_dyn_comp2(TIMESTAMPS[(vis_start+1):], [phi_dot_stm_algo[vis_id,vis_start:,:], phi_dot_stm_deep_hedging[vis_id,vis_start:,:], phi_dot_stm_leading_order[vis_id,vis_start:,:], phi_dot_stm_ground_truth[vis_id,vis_start:,:]], [phi_stm_algo[vis_id,(vis_start+1):,:], phi_stm_deep_hedging[vis_id,(vis_start+1):,:], phi_stm_leading_order[vis_id,(vis_start+1):,:], phi_stm_ground_truth[vis_id,(vis_start+1):,:]], curr_ts, "quad", [train_args["algo"], "deep_hedging", "leading_order", "ground_truth"], comment = comment)
else:
#    Visualize_dyn_comp(TIMESTAMPS[(vis_start+1):], [phi_stm_algo[vis_id,(vis_start+1):,:], phi_stm_deep_hedging[vis_id,(vis_start+1):,:], phi_stm_leading_order[vis_id,(vis_start+1):,:]], curr_ts, "phi", [train_args["algo"], "deep_hedging", "leading_order"], comment = comment)
#    Visualize_dyn_comp(TIMESTAMPS[(vis_start+1):], [phi_dot_stm_algo[vis_id,vis_start:,:], phi_dot_stm_deep_hedging[vis_id,vis_start:,:], phi_dot_stm_leading_order[vis_id,vis_start:,:]], curr_ts, "phi_dot", [train_args["algo"], "deep_hedging", "leading_order"], comment = comment)
    Visualize_dyn_comp2(TIMESTAMPS[(vis_start+1):], [phi_dot_stm_algo[vis_id,vis_start:,:], phi_dot_stm_deep_hedging[vis_id,vis_start:,:], phi_dot_stm_leading_order[vis_id,vis_start:,:]], [phi_stm_algo[vis_id,(vis_start+1):,:], phi_stm_deep_hedging[vis_id,(vis_start+1):,:], phi_stm_leading_order[vis_id,(vis_start+1):,:]], curr_ts, "power", [train_args["algo"], "deep_hedging", "leading_order"], comment = comment)
...
print(f"deep heding loss: {loss_eval_deep_hedging}")
