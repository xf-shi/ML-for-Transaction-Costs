...
Original file is located at
    https://colab.research.google.com/drive/1D9I3h_yjB-ituCQ9KjyH8-ypHWJXHw3P
"""
...
def get_constants_high_dim(dW_std, W_s0d = None):
    ...
    lam_mm = torch.diag(torch.tensor([0.1269, 0.1354, 0.1595])) * 1e-8 * COEF_ #* 0.01
...
def Visualize_dyn_comp(timestamps, arr_lst, ts, name, algo_lst, comment = None):
    ...
    for arr, algo in zip(arr_lst, algo_lst):
        arr = arr.cpu().detach().numpy()
        if ts != "test" or algo != "deep_hedging":
          if len(arr.shape) == 2 and arr.shape[1] == 1:
              arr = arr.reshape((-1,))
          if len(arr.shape) == 1:
...
def evaluation(dW_std, curr_ts, model = None, algo = "deep_hedging", cost = "quadratic", visualize_obs = 0, pasting_cutoff = 0, phi_0 = None, W_s0d = None, is_arr = False, pasting_T = None, time_lst = None, pasting_algo = "deep_hedging"):
...
def transfer_learning(train_args, N_rounds = 5, n_train = 1, n_sample_lst = [128, 128], lr_lst = [1e-3, 1e-4], epoch_lst = [20000, 20000], retrain = False):
    if POWER == 2:
        cost = "quadratic"
    else:
        cost = "power"
...
train_args = {
    ...
    "retrain": False,
    "pasting_cutoff": 2510, #4800,
    "pasting_T": 40, #160, # None
...
if train_args["algo"] == "pasting":
    ...
    model, loss_arr, prev_ts, curr_ts, train_args = transfer_learning(train_args, N_rounds = 5, n_train = 1, n_sample_lst = [128, 500], lr_lst = [1e-3, 1e-4], epoch_lst = [10, 10])
else:
    model, loss_arr, prev_ts, curr_ts = training_pipeline(**train_args)
model.eval()
## TODO: Modify It!!!
...
vis_start = 0#2400#4500
if train_args["algo"] == "pasting":
    day = int(TR - train_args["pasting_cutoff"] / T * TR)
    comment = f"Pasting at last {day} days"
else:
    comment = None
if cost == "quadratic":
    Visualize_dyn_comp(TIMESTAMPS[(vis_start+1):], [phi_stm_algo[0,(vis_start+1):,:], phi_stm_leading_order[0,(vis_start+1):,:], phi_stm_ground_truth[0,(vis_start+1):,:]], curr_ts, "phi", [train_args["algo"], "leading_order", "ground_truth"], comment = comment)
    Visualize_dyn_comp(TIMESTAMPS[(vis_start+1):], [phi_dot_stm_algo[0,vis_start:,:], phi_dot_stm_leading_order[0,vis_start:,:], phi_dot_stm_ground_truth[0,vis_start:,:]], curr_ts, "phi_dot", [train_args["algo"], "leading_order", "ground_truth"], comment = comment)
else:
    Visualize_dyn_comp(TIMESTAMPS[(vis_start+1):], [phi_stm_algo[0,(vis_start+1):,:], phi_stm_leading_order[0,(vis_start+1):,:]], curr_ts, "phi", [train_args["algo"], "leading_order"], comment = comment)
    Visualize_dyn_comp(TIMESTAMPS[(vis_start+1):], [phi_dot_stm_algo[0,vis_start:,:], phi_dot_stm_leading_order[0,vis_start:,:]], curr_ts, "phi_dot", [train_args["algo"], "leading_order"], comment = comment)
