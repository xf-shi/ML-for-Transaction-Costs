...
test_samples=3 #TEST SAMPLE SIZE
...
UtilityLoss_APP_Mathematica=S_OUTSTANDING*Mean_Utility_on_s(XI_test_on_s.cpu(),torch.from_numpy(PHI_APP_Mathematica)/S_OUTSTANDING,
                          torch.from_numpy(PHI_dot_APP_Mathematica)/S_OUTSTANDING,q,S_OUTSTANDING,GAMMA,LAM,MU_BAR,ALPHA,TIME)
...
df=df.append({"Method":"Leading Order","E(Utility)":-UtilityLoss_APP_Mathematica.data.cpu().numpy()/TIME,"sd(Utility)":"{:e}".format(UtilitySD_APP_Mathematica.data.cpu().numpy()/TIME),"MSE at T (on S)":FBSDELoss_APP_Mathematica.data.cpu().numpy()},ignore_index=True)
