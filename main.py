...
class ModelFactory:
    ...
    def save_to_file(self, curr_ts = None):
        ...
        torch.save(model_save, f"{MODELDIR}/{self.algo}_{self.model_name}__{curr_ts}.pt")
        return curr_ts
    ...
    def load_latest(self):
        ...
        model = torch.load(f"{MODELDIR}/{self.algo}_{self.model_name}__{ts}.pt")
...
def visualize_loss(loss_arr, ts, loss_truth):
    ...
    plt.savefig(f"{PLOTDIR}/loss_{ts}.png")
    plt.close()
...
def Visualize_dyn(timestamps, arr, ts, name):
    ...
    plt.title(title)
    plt.legend()
    plt.savefig(f"{PLOTDIR}/{name}_{ts}.png")
    plt.close()
...
def Visualize_dyn_comp(timestamps, arr_lst, ts, name, algo_lst):
    ...
    plt.title(title)
    plt.legend()
    plt.savefig(f"{PLOTDIR}/comp_{name}_{ts}.png")
    plt.close()
...
Visualize_dyn_comp(TIMESTAMPS[1:], [phi_stm_fbsde[0,1:,1], phi_stm_ground_truth[0,1:,1]], curr_ts, "phi", ["fbsde", "ground_truth"])
Visualize_dyn_comp(TIMESTAMPS[1:], [phi_dot_stm_fbsde[0,:,1], phi_dot_stm_ground_truth[0,:,1]], curr_ts, "phi_dot", ["fbsde", "ground_truth"])
Visualize_dyn_comp(TIMESTAMPS[1:], [phi_stm_fbsde[0,1:,2], phi_stm_ground_truth[0,1:,2]], curr_ts, "phi", ["fbsde", "ground_truth"])
Visualize_dyn_comp(TIMESTAMPS[1:], [phi_dot_stm_fbsde[0,:,2], phi_dot_stm_ground_truth[0,:,2]], curr_ts, "phi_dot", ["fbsde", "ground_truth"])
